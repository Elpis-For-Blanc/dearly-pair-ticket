...
function render(){
 ...
 ["love","cinema","train","flight","concert","admission"].forEach(x=>ticket.classList.toggle("type-"+x,x===type));
 ["circle","square","rounded","heart","diamond","arch","stamp"].forEach(x=>ticket.classList.toggle("photo-shape-"+x,x===shape));
 $("stubLabel").textContent=typeMeta[type]||"PAIR PASS";
...
$("cropApply").onclick=()=>{
 ...
 ctx.drawImage(crop.img,sx,sy,sw,sh,0,0,900,900);
 const data=out.toDataURL("image/jpeg",0.9),preview=crop.target.preview,input=crop.target.input;
